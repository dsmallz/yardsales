...
import com.ksl.yardsales.web.AddressController;
...
privileged aspect AddressController_Roo_Controller_Json {
    ...
    @ResponseBody
    public ResponseEntity<String> AddressController.showJson(@PathVariable("id") Long id) {
    ...
    public ResponseEntity<String> AddressController.deleteFromJson(@PathVariable("id") Long id) {
        Address address = addressService.findAddress(id);
        HttpHeaders headers = new HttpHeaders();
        headers.add("Content-Type", "application/json");
        if (address == null) {
